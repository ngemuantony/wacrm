  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        // 1. Get Presigned URL
        const { success, data, error } = await getUploadUrl(`account-${profile.account_id}/${fileName}`, file.type);
        if (!success || !data) {
        ...
        {cfg.media_url ? (
          <div className="flex items-center gap-2 rounded-md border border-border bg-muted px-3 py-2 text-xs">
            ...
            <button
              type="button"
              onClick={handleClear}
              className="rounded p-1 text-muted-foreground hover:bg-slate-700 hover:text-foreground"
          ...
          <button
            ...
            disabled={uploading}
            className="flex w-full items-center justify-center gap-2 rounded-md border border-dashed border-border bg-card/80 backdrop-blur-md shadow-lg px-3 py-4 text-xs text-muted-foreground transition-colors hover:border-slate-600 hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-60"
          >
            {uploading ? (
              <>
