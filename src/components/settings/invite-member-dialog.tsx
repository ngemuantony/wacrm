...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Tuinnov8WaCRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Tuinnov8WaCRM account"
    // doesn't mean much, but also provides a fallback.
    const accountName = result?.accountName ?? 'our Tuinnov8WaCRM account';
    const message = `Join ${accountName} on Tuinnov8WaCRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
  ...
  return (
    ...
    >
      <DialogContent className="bg-card/80 backdrop-blur-md shadow-lg border-border sm:max-w-md">
        {result ? (
          <>
            <DialogHeader>
