...
export async function POST(request: Request) {
  try {
    ...
    // Tuinnov8WaCRM is single-tenant-per-WhatsApp-number — letting two accounts
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one Tuinnov8WaCRM user.',
        },
        { status: 409 }
      )
    }
    ...
    if (needsRegistration) {
      if (!pin) {
        return NextResponse.json(
          {
            error:
              'Two-step verification PIN is required to subscribe this number to Tuinnov8WaCRM. ' +
